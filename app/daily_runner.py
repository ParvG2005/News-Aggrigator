...
def run_daily_pipeline(hours: int = 24, top_n: int = 10) -> dict:
    ...
    try:
        logger.info("\n[1/4] Scraping articles from sources...")
        scraping_results = run_scrapers(hours=hours)
        results["scraping"] = {
            "youtube": len(scraping_results.get("youtube", [])),
            "external": len(scraping_results.get("external", [])),
        }
        logger.info(
            f"✓ Scraped {results['scraping']['youtube']} YouTube videos, "
            f"{results['scraping']['external']} external articles"
        )
        
        logger.info("\n[2/4] Processing YouTube transcripts...")
        ...
        logger.info("\n[3/4] Creating digests for articles...")
        ...
        logger.info("\n[4/4] Generating and sending email digest...")
