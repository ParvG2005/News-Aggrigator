...
from .models import YouTubeVideo, ExternalArticle, AnthropicArticle, Digest
...
class Repository:
    ...
    def create_external_article(self, guid: str, title: str, url: str, published_at: datetime,
                                description: str = "", category: Optional[str] = None) -> Optional[ExternalArticle]:
        existing = self.session.query(ExternalArticle).filter_by(guid=guid).first()
        if existing:
            return None
    ...
    def bulk_create_external_articles(self, articles: List[dict]) -> int:
        new_articles = []
        for a in articles:
            existing = self.session.query(ExternalArticle).filter_by(guid=a["guid"]).first()
            if not existing:
                new_articles.append(ExternalArticle(
                    guid=a["guid"],
                    title=a["title"],
                    url=a["url"],
    ...
    def get_articles_without_digest(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        external_articles = self.session.query(ExternalArticle).all()
        for article in external_articles:
            key = f"external:{article.guid}"
            if key not in seen_ids:
                articles.append({
                    "type": "external",
                    "id": article.guid,
