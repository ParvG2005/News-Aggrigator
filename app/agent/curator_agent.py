...
{pref_text}"""
    ...
    def rank_digests(self, digests: List[dict]) -> List[RankedArticle]:
        if not digests:
            return []
        return self._fallback_ranked(digests)

    def _fallback_ranked(self, digests: List[dict]) -> List[RankedArticle]:
        fallback_ranked = []
        for idx, digest in enumerate(digests, 1):
            fallback_ranked.append(
                RankedArticle(
                    digest_id=digest["id"],
                    relevance_score=max(0.0, 10.0 - (idx - 1) * 0.5),
                    rank=idx,
                    reasoning="Fallback ranking used because AI ranking is unavailable.",
                )
            )
